...
WORKDIR /src
COPY ["BPOR.Rms.Callback.csproj", "BPOR.Rms.Callback/"]
RUN dotnet restore "BPOR.Rms.Callback/BPOR.Rms.Callback.csproj"

WORKDIR "/src/BPOR.Rms.Callback"
COPY . .
RUN dotnet build "BPOR.Rms.Callback.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "BPOR.Rms.Callback.csproj" \
...
Execute unit tests
```
    cd "BPOR.Rms.Callback/test/BPOR.Rms.Callback.Tests"
    dotnet test
```

Deploy application
```
    cd "BPOR.Rms.Callback/src/BPOR.Rms.Callback"
