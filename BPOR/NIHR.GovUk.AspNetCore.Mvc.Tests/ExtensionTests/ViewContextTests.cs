...
public class ViewContextTests
{
    ...
    [Fact]
    public async void Errors_ShouldGenerateOrderedListOfErrors_WhenPropertiesHaveDisplayOrder()
    {
        // Arrange
    ...
    [Fact]
    public async void Errors_ShouldGenerateOrderedListOfErrors_WhenNestedPropertiesExist()
    {
        // Arrange
        var modelState = new ModelStateDictionary();
        modelState.AddModelError("Parent2.Child1", "Error message 1");
        modelState.AddModelError("Parent1.Child2", "Error message 2");
        ...
        Assert.Equal("Parent1.Child2", orderedErrors.First().Key);
        Assert.Equal("Error message 2", orderedErrors.First().Value.Errors.First().ErrorMessage);
        Assert.Equal("Parent2.Child1", orderedErrors.Skip(1).First().Key);
        Assert.Equal("Error message 1", orderedErrors.Skip(1).First().Value.Errors.First().ErrorMessage);
    }
    
    [Fact]
    public async void Errors_ShouldGenerateOrderedListOfErrors_WhenNestedPropertiesExistWithNoDisplayAttribute()
    {
        // Arrange
        var modelState = new ModelStateDictionary();
